...
describe('DemoController', () => {
  ...
  describe('API: "/api/say-hello/:name"', () => {
    ...
    it(`should return a JSON object with the message "${SUC_MSG}" and a status code of 250 if message was successful`, done => {
      agent
        .get('/api/say-hello/seanmaxwell')
        .end((err, res) => {
          if (err) { cerr(err); }
    ...
    it(`should return a JSON object with the message "${ERR_MSG}" and a status code of 400 if message was unsuccessful`, done => {
      agent
        .get('/api/say-hello/makeitfail')
        .end((err, res) => {
          if (err) { cerr(err); }
